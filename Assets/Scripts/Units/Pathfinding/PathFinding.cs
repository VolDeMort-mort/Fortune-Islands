...
namespace FortuneIslands.Units.PathFinding
{
    public static class Pathfinding
    {
        ...
        public static List<Vector2Int> FindPath(CellData[,] grid, Vector2Int startPos, Vector2Int targetPos)
        {
            ...
            // Guard against out-of-bounds requests (e.g. a target computed from a
            // raw world/local position that ended up off the map)
            if (startPos.x < 0 || startPos.x >= width || startPos.y < 0 || startPos.y >= height ||
                targetPos.x < 0 || targetPos.x >= width || targetPos.y < 0 || targetPos.y >= height)
            {
                return null;
            }
