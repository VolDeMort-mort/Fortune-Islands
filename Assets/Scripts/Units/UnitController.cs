...
using FortuneIslands.Units.Commands;
using FortuneIslands.Units.PathFinding;
            ...
            while (target != null && target.currentHealth > 0)
            {
                ...
                if (dist > stats.attackRange)
                {
                    ...
                    if (!_isMoving)
                    {
                        ...
                        // NOTE: grid positions are LOCAL to island.worldContainer, not world space.
                        // Islands are spawned at different world offsets (see GameManager.SpawnIsland),
                        // so using target.transform.position directly was wrong for any island other
                        // than the one at world origin.
                        Vector3 localTargetPos = transform.parent.InverseTransformPoint(target.transform.position);
                        MoveTo(new Vector2Int(Mathf.RoundToInt(localTargetPos.x), Mathf.RoundToInt(localTargetPos.z)));
                    }
